#!/bin/sh
#
# $Id: enableTarget.sh,v 1.1 2002/01/06 21:39:08 racon Exp $
#
# enables a target for packages for and from a specific host.
#
...
if test -z "$1" -o -z "$2" -o -z "$3" ;  then
    ...
    iptables -A FORWARD -s $2 -m mac --mac-source $1 \
             --mac-destination $ROUTER_MAC -j $3 > /dev/null 2>&1;
    test $? -ne 0 && exit 1;
    iptables -A FORWARD -d $2 -m mac --mac-destination $1 \
